<?php
	...
	class AdminController extends Controller{
		...
		//Add NIN Pricing
		public function addNinPricing(){
			extract($_POST);
			$check=$this->model->addNinPricing($slip_type,$user_price,$agent_price,$vendor_price,$status);
			if($check == 0){return $this->createNotification1("alert-success","NIN Pricing Added Successfully");}
			else{return $this->createNotification1("alert-danger","Unable To Add NIN Pricing, Please Try Again");}
		}

		//Update NIN Pricing
		public function updateNinPricing(){
			extract($_POST);
			$check=$this->model->updateNinPricing($id,$user_price,$agent_price,$vendor_price,$status);
			if($check == 0){return $this->createNotification1("alert-success","NIN Pricing Updated Successfully");}
			else{return $this->createNotification1("alert-danger","Unable To Update NIN Pricing, Please Try Again");}
		}

		//----------------------------------------------------------------------------------------------------------------
		//	Verification Pricing Management
		//----------------------------------------------------------------------------------------------------------------

		//Get Verification Pricing
		public function getVerificationPricing(){
			$data=$this->model->getVerificationPricing();
			return $data;
		}

		//Add Verification Pricing
		public function addVerificationPricing(){
			extract($_POST);
			$check=$this->model->addVerificationPricing($service_type,$plan_name,$user_price,$agent_price,$vendor_price,$status);
			if($check == 0){return $this->createNotification1("alert-success","Verification Pricing Added Successfully");}
			else{return $this->createNotification1("alert-danger","Unable To Add Verification Pricing, Please Try Again");}
		}

		//Update Verification Pricing
		public function updateVerificationPricing(){
			extract($_POST);
			$check=$this->model->updateVerificationPricing($id,$service_type,$plan_name,$user_price,$agent_price,$vendor_price,$status);
			if($check == 0){return $this->createNotification1("alert-success","Verification Pricing Updated Successfully");}
			else{return $this->createNotification1("alert-danger","Unable To Update Verification Pricing, Please Try Again");}
		}

		//Delete Verification Pricing
		public function deleteVerificationPricing(){
			$id = base64_decode($_GET["delete-verification-pricing"]);
			$check=$this->model->deleteVerificationPricing($id);
			if($check == 0){return $this->createNotification1("alert-success","Verification Pricing Deleted Successfully");}
			else{return $this->createNotification1("alert-danger","Unable To Delete Verification Pricing, Please Try Again");}
		}

		//----------------------------------------------------------------------------------------------------------------
		// CAC Orders Management
		//----------------------------------------------------------------------------------------------------------------

		//Get All CAC Orders
		public function getCacOrders(){
			$data=$this->model->getCacOrders();
			return $data;
		}

		//Get CAC Order Details
		public function getCacOrderDetails($id){
			$data=$this->model->getCacOrderById($id);
			return $data;
		}

		//Update CAC Order Status
		public function updateCacOrderStatus(){
			extract($_POST);
			$check=$this->model->updateCacOrderStatus($order_id,$status,$notes);
			if($check == 0){
				//Send email notification to user about status change
				$order = $this->model->getCacOrderById($order_id);
				if($order){
					$subject = "CAC Registration #".$order->ref." - ".strtoupper($status);
					$message = "Dear ".$order->sFname.",<br><br>Your CAC registration (Ref: ".$order->ref.") has been updated to: <b>".strtoupper($status)."</b>.<br><br>";
					if($notes != ""){
						$message .= "<b>Admin Notes:</b><br>".nl2br($notes)."<br><br>";
					}
					if($status == "completed"){
						$message .= "Your certificate/document has been uploaded and is ready for download. Please check your transaction details.<br><br>";
					}
					if($status == "rejected"){
						$message .= "Please contact support for more information.<br><br>";
					}
					$message .= "Thank you for choosing ".($this->model->sitename??"us").".";
					$this->model->sendCacEmail($order->sEmail,$subject,$message);
				}
				return $this->createNotification1("alert-success","CAC Order Status Updated Successfully");
			}
			else{return $this->createNotification1("alert-danger","Unable To Update Status, Please Try Again");}
		}

		//Upload CAC Document
		public function uploadCacDocument(){
			$order_id = (float) $_POST["order_id"];
			if(!isset($_FILES["document"]) || $_FILES["document"]["error"] != 0){
				return $this->createNotification1("alert-danger","Please select a valid file to upload.");
			}
			$allowed = ["pdf","doc","docx","jpg","jpeg","png"];
			$ext = strtolower(pathinfo($_FILES["document"]["name"],PATHINFO_EXTENSION));
			if(!in_array($ext,$allowed)){
				return $this->createNotification1("alert-danger","Invalid file type. Allowed: PDF, DOC, DOCX, JPG, PNG.");
			}
			$rootPath = realpath(__DIR__."/../../").DIRECTORY_SEPARATOR;
			$uploadDir = $rootPath."uploads".DIRECTORY_SEPARATOR."cac".DIRECTORY_SEPARATOR;
			if(!file_exists($uploadDir)){mkdir($uploadDir,0777,true);}
			$filename = "cac_".$order_id."_".time().".".$ext;
			$dest = $uploadDir.$filename;
			if(move_uploaded_file($_FILES["document"]["tmp_name"],$dest)){
				$relativePath = "uploads/cac/".$filename;
				$check = $this->model->uploadCacDocument($order_id,$relativePath);
				if($check == 0){
					...
					if($order){
						...
						$subject = "CAC Document Ready for Download";
						$message = "Dear ".$order->sFname.",<br><br>Your CAC document has been uploaded and is ready for download.<br><br><b>Reference:</b> ".$order->ref."<br><b>Business:</b> ".$order->first_choice_name."<br><br>Please log in to your account and check the transaction details to download your document.<br><br>Thank you for choosing ".($this->model->sitename??"us").".";
						$this->model->sendCacEmail($order->sEmail,$subject,$message);
					}
					return $this->createNotification1("alert-success","Document Uploaded Successfully & User Notified.");
				}
				else{return $this->createNotification1("alert-danger","Unable To Save Document, Please Try Again.");}
			}
			else{return $this->createNotification1("alert-danger","File Upload Failed, Please Try Again.");}
		}

		//Send CAC Feedback Email
		public function sendCacFeedback(){
			extract($_POST);
			$check=$this->model->sendCacEmail($email,$subject,$message);
			if($check == 0){return $this->createNotification1("alert-success","Email Message Sent Successfully");}
			else{return $this->createNotification1("alert-danger","Unable To Send Mail, Please Try Again.");}
		}
