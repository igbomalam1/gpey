<?php
	...
	class AdminModel extends Model{
		...
		public function updateApiConfiguration(){
			$dbh=self::connect();
			$count = COUNT($_POST);
			$skip = ["update-api-config","update-monnify-config","update-paystack-config","update-novac-config"];

			if($count > 0){

				foreach($_POST AS $index => $value){
					if(in_array($index,$skip)){ continue; }
					$sql = "INSERT INTO apiconfigs (`name`,`value`) VALUES (:n,:d) ON DUPLICATE KEY UPDATE `value`=:d2";
					$query = $dbh->prepare($sql);
					$query->bindParam(':n',$index,PDO::PARAM_STR);
					$query->bindParam(':d',$value,PDO::PARAM_STR);
					$query->bindParam(':d2',$value,PDO::PARAM_STR);
					$query->execute();
				}
		...
		public function getNinPricing(){
			$dbh=self::connect();
			$sql = "SELECT * FROM nin_pricing ORDER BY id ASC";
            $query = $dbh->prepare($sql);
            $query->execute();
            $results=$query->fetchAll(PDO::FETCH_OBJ);
            return $results;
		...
		//Add NIN Pricing
		public function addNinPricing($slip_type,$user_price,$agent_price,$vendor_price,$status){
            $dbh=self::connect();
            $sql = "INSERT INTO nin_pricing SET slip_type=:st,user_price=:up,agent_price=:ap,vendor_price=:vp,status=:s";
            $query = $dbh->prepare($sql);
            $query->bindParam(':st',$slip_type,PDO::PARAM_STR);
            $query->bindParam(':up',$user_price,PDO::PARAM_STR);
            $query->bindParam(':ap',$agent_price,PDO::PARAM_STR);
            $query->bindParam(':vp',$vendor_price,PDO::PARAM_STR);
            $query->bindParam(':s',$status,PDO::PARAM_STR);
            if($query->execute()){return 0;}else{return 1;}
		}

		//Update NIN Pricing
		public function updateNinPricing($id,$user_price,$agent_price,$vendor_price,$status){
			$dbh=self::connect();
			$id=(float) $id;
			$sql="UPDATE nin_pricing SET user_price=:up,agent_price=:ap,vendor_price=:vp,status=:s WHERE id=$id";
            $query = $dbh->prepare($sql);
            $query->bindParam(':up',$user_price,PDO::PARAM_STR);
            $query->bindParam(':ap',$agent_price,PDO::PARAM_STR);
            $query->bindParam(':vp',$vendor_price,PDO::PARAM_STR);
		...
		//----------------------------------------------------------------------------------------------------------------
		//	Verification Pricing Management (Driver License, Passport, Voter Card, CAC, TIN)
		//----------------------------------------------------------------------------------------------------------------

		//Get Verification Pricing
		public function getVerificationPricing(){
			$dbh=self::connect();
			$sql = "SELECT * FROM verification_pricing ORDER BY service_type ASC, id ASC";
            $query = $dbh->prepare($sql);
            $query->execute();
            $results=$query->fetchAll(PDO::FETCH_OBJ);
            return $results;
		...
		public function addVerificationPricing($service_type,$plan_name,$user_price,$agent_price,$vendor_price,$status){
            $dbh=self::connect();
            $sql = "INSERT INTO verification_pricing SET service_type=:st,plan_name=:pn,user_price=:up,agent_price=:ap,vendor_price=:vp,status=:s";
            $query = $dbh->prepare($sql);
            $query->bindParam(':st',$service_type,PDO::PARAM_STR);
            $query->bindParam(':pn',$plan_name,PDO::PARAM_STR);
            $query->bindParam(':up',$user_price,PDO::PARAM_STR);
            $query->bindParam(':ap',$agent_price,PDO::PARAM_STR);
            $query->bindParam(':vp',$vendor_price,PDO::PARAM_STR);
            $query->bindParam(':s',$status,PDO::PARAM_STR);
		...
		public function updateVerificationPricing($id,$service_type,$plan_name,$user_price,$agent_price,$vendor_price,$status){
			$dbh=self::connect();
			$id=(float) $id;
			$sql="UPDATE verification_pricing SET service_type=:st,plan_name=:pn,user_price=:up,agent_price=:ap,vendor_price=:vp,status=:s WHERE id=$id";
            $query = $dbh->prepare($sql);
            $query->bindParam(':st',$service_type,PDO::PARAM_STR);
            $query->bindParam(':pn',$plan_name,PDO::PARAM_STR);
            $query->bindParam(':up',$user_price,PDO::PARAM_STR);
		...
		public function deleteVerificationPricing($id){
			$dbh=self::connect();
			$id=(float) $id;
			$sql="DELETE FROM verification_pricing WHERE id=$id";
            $query = $dbh->prepare($sql);
            if($query->execute()){return 0; } else {return 1; }
		}
		...
		public function getCacOrders(){
			$dbh=self::connect();
			$sql="SELECT c.*,s.sFname,s.sLname,s.sEmail,s.sPhone FROM cac_registrations c JOIN subscribers s ON c.user_id=s.sId ORDER BY c.id DESC";
			$query=$dbh->prepare($sql);
			$query->execute();
			$results=$query->fetchAll(PDO::FETCH_OBJ);
			return $results;
		...
		//Get CAC Order By ID
		public function getCacOrderById($id){
			$dbh=self::connect();
			$id=(float) $id;
			$sql="SELECT c.*,s.sFname,s.sLname,s.sEmail,s.sPhone,s.sWallet FROM cac_registrations c JOIN subscribers s ON c.user_id=s.sId WHERE c.id=$id";
			$query=$dbh->prepare($sql);
			$query->execute();
			$result=$query->fetch(PDO::FETCH_OBJ);
			return $result;
		...
		public function updateCacOrderStatus($id,$status,$notes){
			$dbh=self::connect();
			$id=(float) $id;
			$date=date("Y-m-d H:i:s");
			$sql="UPDATE cac_registrations SET status=:st,admin_notes=:an,updated_at=:ua WHERE id=$id";
			$query=$dbh->prepare($sql);
			$query->bindParam(':st',$status,PDO::PARAM_STR);
			$query->bindParam(':an',$notes,PDO::PARAM_STR);
			$query->bindParam(':ua',$date,PDO::PARAM_STR);
			if($query->execute()){return 0;}else{return 1;}
		}

		//Upload CAC Document
		public function uploadCacDocument($id,$filepath){
			$dbh=self::connect();
			$id=(float) $id;
			$date=date("Y-m-d H:i:s");
			$sql="UPDATE cac_registrations SET document_path=:dp,updated_at=:ua WHERE id=$id";
			$query=$dbh->prepare($sql);
			$query->bindParam(':dp',$filepath,PDO::PARAM_STR);
			$query->bindParam(':ua',$date,PDO::PARAM_STR);
			if($query->execute()){return 0;}else{return 1;}
		}

		//Send CAC Email Notification
		public function sendCacEmail($email,$subject,$message){
			$subject = $subject ." (".$this->sitename.")";
			$mailResult = self::sendMail($email,$subject,$message);
			return 0;
		}
