<?php
	...
	class ApiModel extends Model{
        ...
        //Validate Novac Transaction
        public function verifyNovacRef($email,$reference){
			$dbh=self::connect();

            $sql = "SELECT value FROM apiconfigs WHERE name='novacSecretKey'";
            $query = $dbh->prepare($sql);
            $query->execute();
            $result=$query->fetch(PDO::FETCH_OBJ);
            $apiKey=$result->value;

            $sql2 = "SELECT value FROM apiconfigs WHERE name='novacCharges'";
            $query2 = $dbh->prepare($sql2);
            $query2->execute();
            $result2=$query2->fetch(PDO::FETCH_OBJ);
            $charges=$result2->value;
            ...
            $curl = curl_init();
            curl_setopt_array($curl, array(
                    CURLOPT_URL => "https://api.novacpayment.com/api/v1/checkout/" . rawurlencode($reference) . "/verify",
                    CURLOPT_RETURNTRANSFER => true,
                    CURLOPT_HTTPHEADER => [
                        "Accept: application/json",
                        "Authorization: Bearer ".$apiKey,
            ...
            if(!isset($tranx['status']) || !$tranx['status']){
                    return 'API Returned Error: ' . (isset($tranx['message']) ? $tranx['message'] : 'Unknown Error');
            }

            if(isset($tranx['data']['status']) && $tranx['data']['status'] == 'successful'){
                 ...
                 return "Transaction Not Verified. Status: " . (isset($tranx['data']['status']) ? $tranx['data']['status'] : 'Unknown');
            }
		}
