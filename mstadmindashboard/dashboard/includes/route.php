<?php
    ...
    */
    ...
    //Update Novac Configurations
    if(isset($_POST["update-novac-config"])){
        $msg=$controller->updateApiConfiguration();
    }
    ...
    //Add NIN Pricing
    if(isset($_POST["add-nin-pricing"])){
        $msg=$controller->addNinPricing();
    }

    //Update NIN Pricing
    if(isset($_POST["update-nin-pricing"])){
        $msg=$controller->updateNinPricing();
    }

    //Add Verification Pricing
    if(isset($_POST["add-verification-pricing"])){
        $msg=$controller->addVerificationPricing();
    }

    //Update Verification Pricing
    if(isset($_POST["update-verification-pricing"])){
        $msg=$controller->updateVerificationPricing();
    }

    //Delete Verification Pricing
    if(isset($_GET["delete-verification-pricing"])){
        $msg=$controller->deleteVerificationPricing();
        exit();
    }
	...
	if(isset($_POST["update-cac-status"])){
		$msg=$controller->updateCacOrderStatus();
	}

	//Upload CAC Document
	if(isset($_POST["upload-cac-document"])){
		$msg=$controller->uploadCacDocument();
	}

	//Send CAC Feedback Email
	if(isset($_POST["send-cac-email"])){
		$msg=$controller->sendCacFeedback();
	}
    ...
    function getDataIfAny($page){
        ...
        switch($page){
            ...
            case "nin-pricing":
                return $controller->getNinPricing();
                break;

		case "verification-pricing":
				return $controller->getVerificationPricing();
				break;

		case "cac-orders":
				return $controller->getCacOrders();
				break;

		case "cac-order-details":
				if(isset($_GET["id"])){
					return $controller->getCacOrderDetails($_GET["id"]);
				}
				else{header("Location: cac-orders");}
				break;
